#!/usr/bin/env bash

set -euo pipefail

export PATH="${HOME}/.cargo/bin":"${HOME}/.npm/bin":"${HOME}/.local/bin":"${HOME}/.local/sbin":"${HOME}/bin":"${HOME}/sbin":"/opt/homebrew/bin":"/opt/homebrew/sbin":${PATH}

if ! type brew >/dev/null 2>&1; then
  bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"
  test -d /opt/homebrew/etc && chmod go-w /opt/homebrew/etc
fi

brew install --formula git
brew install --formula gh

GITDIR="${HOME}/.dotfiles"

if [ ! -d "${GITDIR}" ]; then
  gh auth login
  gh repo clone dotfiles "${GITDIR}" -- --recursive
fi

mkdir -pv "${HOME}/.config"

ln -fsv "${GITDIR}/.aliases"               "${HOME}/.aliases"
ln -fsv "${GITDIR}/.config/bat"            "${HOME}/.config/bat"
ln -fsv "${GITDIR}/.config/fd"             "${HOME}/.config/fd"
ln -fsv "${GITDIR}/.config/git"            "${HOME}/.config/git"
ln -fsv "${GITDIR}/.config/lf"             "${HOME}/.config/lf"
ln -fsv "${GITDIR}/.config/pip"            "${HOME}/.config/pip"
ln -fsv "${GITDIR}/.config/rg"             "${HOME}/.config/rg"
ln -fsv "${GITDIR}/.config/sheldon"        "${HOME}/.config/sheldon"
ln -fsv "${GITDIR}/.config/starship.toml"  "${HOME}/.config/starship.toml"
ln -fsv "${GITDIR}/.config/tig"            "${HOME}/.config/tig"
ln -fsv "${GITDIR}/.config/vim"            "${HOME}/.config/vim"
ln -fsv "${GITDIR}/.curlrc"                "${HOME}/.curlrc"
ln -fsv "${GITDIR}/.inputrc"               "${HOME}/.inputrc"
ln -fsv "${GITDIR}/.npm-rc"                "${HOME}/.npmrc"
ln -fsv "${GITDIR}/.profile"               "${HOME}/.profile"
ln -fsv "${GITDIR}/.ripgreprc"             "${HOME}/.ripgreprc"
ln -fsv "${GITDIR}/.textlintrc"            "${HOME}/.textlintrc"
ln -fsv "${GITDIR}/.tmux.conf"             "${HOME}/.tmux.conf"
ln -fsv "${GITDIR}/.w3m"                   "${HOME}/.w3m"
ln -fsv "${GITDIR}/.wgetrc"                "${HOME}/.wgetrc"
ln -fsv "${GITDIR}/.zsh"                   "${HOME}/.zsh"
ln -fsv "${GITDIR}/.zshenv"                "${HOME}/.zshenv"

GITDIR="${HOME}/.conceal"

if [ ! -d "${GITDIR}" ]; then
  gh repo clone dotfiles.conceal "${GITDIR}" -- --recursive
  chmod -Rv go-rwx "${GITDIR}/.ssh"
fi

ln -fsv "${GITDIR}/.config/openai.token"   "${HOME}/.config/openai.token"
ln -fsv "${GITDIR}/.config/textlint"       "${HOME}/.config/textlint"
ln -fsv "${GITDIR}/.ssh"                   "${HOME}/.ssh"
ln -fsv "${GITDIR}/.w3m/bookmark.html"     "${HOME}/.w3m/bookmark.html"
ln -fsv "${GITDIR}/intelephense"           "${HOME}/intelephense"

brew install --cask 1password
brew install --cask adguard
brew install --cask adguard-vpn
brew install --cask alfred
brew install --cask altserver
brew install --cask appcleaner
brew install --cask iterm2
brew install --cask karabiner-elements
brew install --cask logitech-g-hub
brew install --cask microsoft-edge
brew install --cask microsoft-office
brew install --cask postman
brew install --cask sequel-ace
brew install --cask visual-studio-code

brew install --formula cmake
brew install --formula composer
brew install --formula coreutils
brew install --formula docker
brew install --formula docker-completion
brew install --formula fzf
brew install --formula gawk
brew install --formula git-flow
brew install --formula go
brew install --formula jq
brew install --formula lf
brew install --formula mysql
brew install --formula node
brew install --formula p7zip
brew install --formula pandoc
brew install --formula poppler
brew install --formula tig
brew install --formula tmux
brew install --formula translate-shell
brew install --formula universal-ctags
brew install --formula vim
brew install --formula w3m
brew install --formula wget
brew install --formula xz

if ! type cargo >/dev/null 2>&1; then
  curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh
fi

cargo install bat
cargo install cargo-update
cargo install fd-find
cargo install git-delta
cargo install lsd
cargo install ripgrep
cargo install sheldon
cargo install starship
cargo install zoxide

export N_PREFIX="${HOME}/.npm"

mkdir -pv "${HOME}/.npm/lib"

npm --global install n
npm --global install corepack
npm --global install textlint
npm --global install textlint-rule-preset-ja-technical-writing
npm --global install textlint-rule-prh

n install lts

if [ ! -f ${HOME}/.local/bin/pip ]; then
  python3 -m venv ${HOME}/.local
fi

pip install pip3-autoremove
pip install pip_search
pip install httpie

if brew list --formula node >/dev/null 2>&1; then
  brew uninstall --formula node
  brew autoremove
fi

if [ ! -x "${HOME}/.tmux/plugins/tpm/bin/install_plugins" ]; then
  git clone https://github.com/tmux-plugins/tpm.git "${HOME}/.tmux/plugins/tpm"
fi

"${HOME}/.tmux/plugins/tpm/bin/install_plugins"
